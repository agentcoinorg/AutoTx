...
def build(user_proxy: UserProxyAgent, agents_information, smart_account_addr: str, network_name: str, get_llm_config: Callable[[],dict]) -> AutogenAgent:
    clarifier_agent = AssistantAgent(
        ...
        system_message=dedent(
            ...
            If the goal is invalid, unsupported or outside the scope of the agents, you MUST call the goal_not_supported tool.
    ...
    def goal_not_supported(
        message: Annotated[str, "The message return to the user about why the goal is not supported"],
    ) -> str:
        cprint(f"Goal not supported: {message}", "red")
        return "TERMINATE"

    clarifier_agent.register_for_llm(name="goal_not_supported", description="Notify the user about their goal not being achievable with current agents")(request_user_input)
    user_proxy.register_for_execution(name="goal_not_supported")(goal_not_supported)
