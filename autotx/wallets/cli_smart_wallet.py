...
class CliSmartWallet(SmartWallet):
    external_wallet: SmartWallet | None

    def __init__(self, web3: Web3, wallet: SmartWallet, task_id: str, tasks: list[models.Task]):
        super().__init__(web3, wallet.address if isinstance(wallet, SmartWallet) else wallet)
    ...
    def on_transactions_ready(self, txs: list[models.Transaction]) -> bool | str:
        if self.external_wallet:
            return self.external_wallet.on_transactions_ready(txs)
