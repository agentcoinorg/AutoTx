...
@tool("Queries the Uniswap pool for a given token pair to obtain necessary parameters for a swap.")
...
@tool("Encodes a swap transaction for a specified amount of input tokens to output tokens using a Uniswap pool.")
def encode_swap(pool_address: str, amount_in: int, slippage_tolerance: float, recipient_address: str, deadline: int) -> str:
    """
...
@tool("Approve token to swap")
def approve_token(amount: int, token_address: str) -> str:
    """
    Encodes token approval with amount in decimals for given token
    
    Args:
        amount (int): The amount in decimals of input tokens to swap.
        token_address(str): The address of the token to swap
    Returns:
        str: The encoded data for the approve transaction, ready to be signed and sent.
    
    Description:
        This tool takes the necessary parameters for encoding token approval before doing the swap
    """

    return "0x_APPROVE_CALLDATA"

default_tools = [query_pools, encode_swap, approve_token]

class UniswapAgent(Agent):
    def __init__(self) -> Agent:
        config: AgentConfig = agents_config["uniswap"].model_dump()
        super().__init__(
            **config,
            tools=default_tools,
            llm=open_ai_llm,
